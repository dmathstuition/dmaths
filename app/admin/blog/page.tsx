...
import type { BlogPost, BlogSubscriber, BlogComment } from "@/lib/blog";
...
export default async function AdminBlogPage() {
  ...
  const [{ data: posts }, { data: subs }, { data: comments }] = await Promise.all([
    ...
    admin.from("blog_comments").select("*").order("created_at", { ascending: false }),
  ]);
  return (
    <BlogAdminClient
      initialPosts={(posts ?? []) as BlogPost[]}
      subscribers={(subs ?? []) as BlogSubscriber[]}
