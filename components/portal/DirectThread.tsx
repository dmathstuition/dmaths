...
type Message = {
  ...
  body: string; audio_url?: string | null; tutor_id?: string | null;
  read: boolean; created_at: string;
};

// A reusable realtime 1:1 thread. Loads the thread (student_id + tutor_id),
// delivers new messages / deletes instantly over `channel`, marks the other
// party's messages read, and reconciles every 45s. Used for learner↔tutor
// threads on both the learner side and the tutor side.
export default function DirectThread({ channel, studentId, tutorId, meRole, otherName, sendParams }: {
  channel: string;
  studentId: string;
  tutorId: string;
  meRole: "student" | "tutor";
  otherName: string;
  sendParams: Record<string, any>; // extra body for /api/messages/send
}) {
  ...
  async function load(reconcile = false) {
    ...
    setMessages(prev => (prev.length === data.length && prev.every((m, i) => m.id === data[i].id)) ? prev : data);
  }
  ...
  useEffect(() => {
    ...
    supabase.from("messages").update({ read: true })
      .eq("student_id", studentId).eq("tutor_id", tutorId)
      .neq("sender_role", meRole).eq("read", false).then(() => {});
  ...
  useEffect(() => {
    ...
    ch.current = c;
    return () => { clearTimeout(typingTimeout.current); supabase.removeChannel(c); };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [channel]);
  ...
  function broadcastTyping() {
    ...
    ch.current?.send({ type: "broadcast", event: "typing", payload: { role: meRole } });
  }
  ...
  async function send() {
    ...
    const res = await fetch("/api/messages/send", {
      ...
      body: JSON.stringify({ ...sendParams, body }),
    });
    const json = await res.json();
    setSending(false);
    if (!res.ok) { push(json.error || "Could not send.", "error"); return; }
    upsert(json.message);
    ch.current?.send({ type: "broadcast", event: "message", payload: { message: json.message } });
    setText("");
  }
  ...
  async function remove(id: string) {
    ...
    ch.current?.send({ type: "broadcast", event: "delete", payload: { id } });
  }

  return (
    <div className="flex h-[60vh] flex-col overflow-hidden rounded-xl border border-line bg-chalk/40">
      <div className="flex-1 space-y-3 overflow-y-auto p-4">
        {messages.length === 0 && <p className="py-10 text-center text-sm text-ink/40">No messages yet. Say hello 👋</p>}
        {messages.map(m => {
        ...
        {peerTyping && (
          <div className="flex items-center gap-1.5 px-1">
            <span className="text-[11px] font-bold text-gold-deep">{otherName} is typing</span>
      ...
      <div className="flex items-end gap-2 border-t border-line p-3">
        <textarea className="field max-h-32 flex-1 resize-none" rows={1} placeholder={`Message ${otherName}…`}
          value={text}
