...
export default function ReportCardsClient({ students, classes, issued }: { students: Student[]; classes: ClassRow[]; issued: Card[] }) {
  ...
  async function issue() {
    ...
    if (mode === "class" && !confirm("Issue a report card to every active student in the class?")) return;
    setBusy(true);
    ...
    const n = j.issued ?? 1;
    push(`${n} report card${n === 1 ? "" : "s"} issued — student${n === 1 ? "" : "s"} notified.`, "success");
    setF({ studentId: "", classId: "", term: f.term, remark: "" });
    router.refresh();
  }

  async function revoke(id: string) {
    if (!confirm("Revoke this report card?")) return;
    const res = await fetch(`/api/report-cards?id=${id}`, { method: "DELETE" });
    if (!res.ok) { push("Could not revoke it.", "error"); return; }
    push("Report card revoked.", "success");
    router.refresh();
  }
  ...
  async function bulkRevoke() {
    const ids = sel.list;
    if (!ids.length || !confirm(`Revoke ${ids.length} report card${ids.length === 1 ? "" : "s"}?`)) return;
    setBulkBusy(true);
    const results = await Promise.all(ids.map((id) => fetch(`/api/report-cards?id=${id}`, { method: "DELETE" })));
    setBulkBusy(false);
    const failed = results.filter((r) => !r.ok).length;
    push(failed ? `${ids.length - failed} revoked, ${failed} failed.` : `${ids.length} report card${ids.length === 1 ? "" : "s"} revoked.`, failed ? "error" : "success");
    sel.clear();
    router.refresh();
  }
    ...
    <div className="space-y-6">
      <div className="boardgrid relative flex items-center gap-4 overflow-hidden rounded-2xl bg-board p-7 text-white">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-white/50">Issue a termly progress report — students and parents download it as a PDF.</p>
        </div>
      </div>
      ...
      <div className="card p-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            {mode === "student" ? (
              <>
                <label htmlFor="report-student" className="flabel">Student</label>
                <select id="report-student" className="field" value={f.studentId} onChange={e => setF({ ...f, studentId: e.target.value })}>
              ...
              <>
                <label htmlFor="report-class" className="flabel">Class</label>
                <select id="report-class" className="field" value={f.classId} onChange={e => setF({ ...f, classId: e.target.value })}>
                  <option value="">Choose a class…</option>
                  {classes.map(c => <option key={c.id} value={c.id}>{c.subject} · {fmtWAT(c.starts_at)}</option>)}
                </select>
                <p className="mt-1 text-[11px] text-ink/45">One report card per active student in the class.</p>
              </>
            )}
          </div>
          <div>
            <label htmlFor="report-term-period" className="flabel">Term / period</label>
            <input id="report-term-period" className="field" list="rc-terms" value={f.term}
          ...
          <div className="sm:col-span-2">
            <label htmlFor="report-remark-optional" className="flabel">Remark <span className="text-ink/40">(optional)</span></label>
            <textarea id="report-remark-optional" className="field min-h-[80px]" value={f.remark} maxLength={600}
              onChange={e => setF({ ...f, remark: e.target.value })}
              placeholder="A strong term — excellent progress in algebra; keep working on timed practice." />
            <p className="mt-1 text-[11px] text-ink/40">Score, attendance, points &amp; conduct are captured automatically from the student&apos;s current record.</p>
          </div>
        </div>
      ...
      <BulkBar count={sel.size} noun="report card" onClear={sel.clear}>
          ...
          <div className="divide-y divide-line/60">
            {issued.map(c => (
              <div key={c.id} className={`flex items-center gap-3 px-5 py-3.5 ${sel.has(c.id) ? "bg-gold-pale/40" : ""}`}>
                <input type="checkbox" className="h-4 w-4 flex-shrink-0" aria-label="Select this report card"
                ...
                <button onClick={() => revoke(c.id)} className="flex-shrink-0 rounded-lg px-2 py-1.5 text-xs font-bold text-red-500 hover:bg-red-50">Revoke</button>
              </div>
            ))}
          </div>
        ) : (
