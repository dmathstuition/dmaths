...
export default function Logo({ light, size = "md", mark = false, className = "" }: { light?: boolean; size?: "sm" | "md" | "lg"; mark?: boolean; className?: string }) {
  ...
  return (
    <Image
      ...
      {...dims}
      priority
      className={`${h} w-auto ${light ? "brightness-0 invert" : ""} ${className}`}
    />
  );
}
