...
function inline(text: string): ReactNode[] {
  ...
  while ((m = re.exec(text))) {
    ...
    if (m[1]) {
      nodes.push(
        <a key={key++} href={m[3]} target="_blank" rel="noopener noreferrer" className="font-semibold text-gold-deep underline underline-offset-2 hover:text-gold">
          {m[2]}
        </a>,
      );
    } else if (m[4]) {
      nodes.push(<strong key={key++} className="font-bold text-ink">{m[5]}</strong>);
    } else if (m[6]) {
      nodes.push(<em key={key++}>{m[7]}</em>);
    } else if (m[8]) {
      nodes.push(<code key={key++} className="rounded bg-chalk px-1.5 py-0.5 text-[0.9em] font-semibold text-ink/80">{m[9]}</code>);
    }
...
export default function BlogContent({ body }: { body: string }) {
  ...
  for (const raw of lines) {
    ...
    if (h3) { flushPara(); flushList(); blocks.push(<h3 key={k++} className="mt-8 font-display text-lg font-bold text-ink">{inline(h3[1])}</h3>); }
